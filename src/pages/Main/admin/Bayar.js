...
const Bayar = (props) => {
  ...
  const bayar = async () => {
    if (
      ...
      const res = await updateDoc(doc(fs, "riwayat", id), {
        status_bayar: true,
      });
      if (res) {
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: "#fff" }}>
      {!data.pembayaran.status_bayar && (
        <Fab
          ...
          size="sm"
          icon={
            <Icon color="white" as={MaterialIcons} name="money" size="sm" />
          }
        />
      )}
      <ScrollView>
        <VStack p={5}>
          ...
          {data.pembayaran.status_bayar ? (
            ...
            <Fragment>
              <FormControl>
                <FormControl.Label>Dibayar</FormControl.Label>
                <Input value={dibayar} onChangeText={setDibayar} />
              </FormControl>
              <FormControl>
                <FormControl.Label>Kembalian</FormControl.Label>
                <Input
                  value={
                    "Rp. " +
                    (
                      parseInt(dibayar) -
                      (parseInt(data.pembayaran.admin) +
                        parseInt(totalHargaObat))
                    ).toString()
                  }
                  onChangeText={setDibayar}
                />
